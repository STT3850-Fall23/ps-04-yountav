...
-   The relationship in @fig-TSG appears to be positive and reasonably linear, with the exception of a few outliers. The graph is mostly skewed right. 
:::
...
-   No, because this graph only demonstrates that the more white households in poverty there are, the more this will cause an increase in Trump supporters. This does not demonstrate a correlation between Trump's policies and their benefit to white impoverished households or that increasing poverty will necessarily benefit the Trump campaign. 
:::
...
-   Using the least squares regression line of the `trump_support` and `poverty` relationship, the predicted percentage of Trump support at 6% white poverty is about 43% Trump support. 
:::
...
-   No, because using this same least squares regression line, the relationship between `trump_support` and `poverty` at 85% poverty will be over 200%, which is way over the max percentage of 100%. A new model would need to be used for this white poverty level. The model created used a maximum of 20% white poverty. 
:::
...
-   I expected the correlation between trump supporters and non-white voters/households to be negative, implying that as trump support increases, the number of non-white voters voting for trump decreases. 
...
-   When the percentage of non-white voters is at 0, the predicted average Trump support is 58.33%. 

-   When the percentage of non-white voters decreases by half a percentage point, there is also a decrease in the estimated Trump support by 0.269 percentage points. 
:::
...
$$\widehat{\text{trump\_support}} = `r round(coef(nw_mod)[1],4)` + `r round(coef(nw_mod)[2],4)` \times \text{non_white}$$
```{r}
predict(nw_mod, newdata = data.frame(non_white = 70))
```

-   The numerical prediction for the level of Trump support in a region that has a population with 70% identifying as a person of color is 39.49%.
:::
...
-   Based on the evidence above and used in previous questions, the better explanatory variable of Trump support is the `poverty` variable. 
:::
...
**A.** If Representative Ocasio-Cortez saw the regression line `nw_mod` without seeing any of the data points, her prediction of Trump support for a region in which 61% of people identify as non-white would be around 43%, give or take. 
...
**B.** Her prediction given the regression line would be about 43%. However, using the data points, the actual data demonstrates that at a 61% `non-white` population, the Trump support is at about 34%. This is a overestimation by about 9 percentage points. 

$$\text{residual} = y - \hat{y}$$

**C.** Using the residual, the predicted value using the regression line is about 43% and the value found using the data points is about 34% given that the population of non-white voters is 61%. So, the residual is -9. 
:::
