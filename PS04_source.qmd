...
-   The relationship in Figure 1 appears to be positive and reasonably linear, with the exception of a few outliers. The graph is mostly skewed right. 
:::
...
-   No, because using this same least squares regression line, the relationship between `trump_support` and `poverty` at 85% poverty will be over 200%, which is way over the max percentage of 100%. A new model would need to be used for this white poverty level. 
:::
